...
const ImageWithErrorBoundary: React.FC<{
  ...
  useEffect(() => {
    ...
    // Validate URL: must be a non-empty string starting with http(s) and not undefined/null
    if (
      !src ||
      typeof src !== "string" ||
      src.trim() === "" ||
...
const HotelCard = ({ province }: { province?: string }) => {
  ...
  useEffect(() => {
    const fetchHotels = async () => {
      try {
        setLoading(true);
        const fetched = await getFeaturedHotels(province);
  ...
  const handleNext = (id: string, images: string[]) =>
    setCurrentImage((prev) => ({ ...prev, [id]: (prev[id] + 1) % images.length }));

  const handlePrev = (id: string, images: string[]) =>
    setCurrentImage((prev) => ({ ...prev, [id]: prev[id] === 0 ? images.length - 1 : prev[id] - 1 }));

  const getGalleryImages = (g?: string, primary?: string) => {
    const images: string[] = [];
    if (primary && typeof primary === "string" && primary.match(/^https?:\/\/[^\s/$.?#].[^\s]*$/)) {
      images.push(primary.trim());
    }
    if (g && typeof g === "string") {
      images.push(...g.split(",").map((s) => s.trim()).filter((s) => s.match(/^https?:\/\/[^\s/$.?#].[^\s]*$/)));
    }
  ...
  const getAmenities = (a?: string) => {
    ...
    return a && typeof a === "string" ? a.split(",").map((id) => map[id] || id).filter(Boolean) : [];
  };
  ...
  const getDescription = (d?: string) => {
    ...
    return clean.length > 150 ? clean.slice(0, clean.lastIndexOf(" ", 150)) + "..." : clean;
  };
  ...
  const getPaginationButtons = () => {
    ...
    if (startPage > 1) {
      ...
      if (startPage > 2) {
        buttons.push(
          <span key="start-ellipsis" className="w-10 h-10 flex items-center justify-center text-gray-700">
            ...
          </span>
        );
      }
    ...
    if (endPage < totalPages) {
      if (endPage < totalPages - 1) {
        buttons.push(
          <span key="end-ellipsis" className="w-10 h-10 flex items-center justify-center text-gray-700">
            ...
          </span>
        );
      }
  ...
  if (hotels.length === 0) return <div className="text-center p-6">No hotels found.</div>;

  return (
    <section className="p-6 max-w-[84rem] mx-auto">
      <h1 className="text-[48px] font-normal font-[MontserratSemiBold] text-center md:text-left mt-4 text-[#212529]">
        Our Hotels & Resorts
      </h1>

      {currentHotels.map((hotel) => {
        const images = getGalleryImages(hotel.sectionData?.Company?.gallery_image, hotel.sectionData?.Company?.primary_image);
        ...
        return (
          ...
          >
            <div className="relative w-full sm:w-[40%] h-[240px] sm:h-[280px] md:h-[300px]">
            ...
            <div className="w-full sm:flex-1 p-5 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center mt-1">
                  ...
                  <span className="text-sm text-gray-600 ml-2">{hotel.rating || "N/A"}</span>
                  <span className="text-sm text-green-600 ml-2">{hotel.location || "Unknown Location"}</span>
                </div>
              ...
              <div className="flex justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-5 flex-wrap">
                    {amenities.length > 0 ? (
                      ...
                      <span className="text-sm text-gray-600">No amenities available</span>
                    )}
                  </div>
                </div>
                ...
                <div className="text-right mt-4">
                  ...
                  <div className="flex justify-end">
                    <div className="text-lg text-gray-400 line-through">$ {hotel.price || 0}</div>
                    <div className="text-lg font-bold text-yellow-500 ml-1">
                  ...
                  <h2 className="text-2xl font-bold text-blue-600">$ {hotel.discountPrice || 0}</h2>
                </div>
              </div>
            </div>
          </div>
